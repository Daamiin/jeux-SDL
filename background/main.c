...
int main(){
	...
	//position mte3 player w loading mte3ou
	SDL_Surface *player;
        ...
        while (SDL_PollEvent(&event)) {
            switch (event.type) {
                case SDL_QUIT:
                    quitter = 1;
                    break;
			case SDL_KEYDOWN: //gestion de mouvement exemple temporelle jusqu'on fait l'integration
				//perso1
			...
			break;
			case SDL_KEYUP: //gestion de mouvement exemple temporelle jusqu'on fait l'integration
				//perso1				
				if (event.key.keysym.sym == SDLK_RIGHT)
				{
						direction_x = 0;	
